...
export async function GET(_request: Request, context: { params: Promise<{ type: string }> }) {
  ...
  if (type === "usuarios") {
    ...
    return responseCsv(`convniver-usuarios-${date}.csv`, [
  ...
  if (type === "convites") {
    ...
    return responseCsv(`convniver-convites-${date}.csv`, [
      ["Convite", "Aniversariante", "Proprietário", "Status", "Data do evento", "Cobrança", "Valor", "Criado em"],
  ...
  if (type === "rsvps") {
    ...
    return responseCsv(`convniver-presencas-${date}.csv`, rows);
  }
  ...
  if (type === "financeiro") {
    ...
    const emails = new Map((profiles ?? []).map((item) => [item.id, item.email]));
    const titles = new Map((invitations ?? []).map((item) => [item.id, item.event_title]));
    const rows: unknown[][] = [["Tipo", "Convite", "Proprietário", "Pagador/Convidado", "E-mail", "Valor", "Taxa CONVNIVER", "Status", "ID pagamento", "Data"]];
    ...
    return responseCsv(`convniver-financeiro-${date}.csv`, rows);
  }
