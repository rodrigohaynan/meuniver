  ...
  return (
    <main className="min-h-screen bg-[#f6f2ef] text-[#351820]">
      <header className="sticky top-0 z-50 border-b border-[#e6d9d2] bg-[#f6f2ef]/95 backdrop-blur">
        <div className="mx-auto flex max-w-7xl flex-col gap-3 px-5 py-3 sm:flex-row sm:items-center sm:justify-between sm:px-8">
          ...
          <div className="flex flex-wrap items-center justify-between gap-2 sm:justify-end">
            <Link href="/painel" className="rounded-full border border-[#dccdc5] bg-white px-4 py-2 text-sm font-bold text-[#684f55]"><span className="sm:hidden">Meu painel</span><span className="hidden sm:inline">Painel do usuário</span></Link>
            <LogoutButton />
          </div>
        </div>
      </header>
      ...
      <div className="mx-auto grid max-w-7xl gap-6 px-5 py-6 sm:px-8 lg:grid-cols-[220px_1fr]">
        <aside className="h-fit min-w-0 rounded-[1.5rem] border border-[#e3d6cf] bg-white p-2 shadow-sm sm:p-3">
          <nav className="flex min-w-0 gap-1 overflow-x-auto pb-1 text-sm font-bold lg:block lg:space-y-1 lg:overflow-visible lg:pb-0">
          ...
          <div className="mt-4 hidden border-t border-[#eee4de] pt-4 text-xs leading-5 text-[#8b767b] lg:block">
...
function AdminLink({ href, icon, children }: { href: string; icon: React.ReactNode; children: React.ReactNode }) {
  return <Link href={href} className="flex shrink-0 items-center gap-2 whitespace-nowrap rounded-xl px-3 py-2.5 text-[#684f55] transition hover:bg-[#f8efeb] hover:text-[#7d1f37]">{icon}{children}</Link>;
}
