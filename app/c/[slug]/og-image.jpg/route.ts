...
async function fallbackCanvas(title: string) {
...
async function toCompactJpeg(image: SharpPipeline) {
  for (const quality of [72, 64, 56, 48, 42]) {
    ...
    if (output.length <= TARGET_BYTES || quality === 42) return output;
  }
  ...
  if (invitation?.hero_image_url?.trim()) {
    try {
      ...
      pipeline = source
        ...
        : await fallbackCanvas(title);
    } catch {
      pipeline = await fallbackCanvas(title);
    }
  } else {
    pipeline = await fallbackCanvas(title);
  }

  const jpeg = await toCompactJpeg(pipeline);
